...
- **RC-3 transition effect centralization** (architecture review, 2026-07-09): `applyTransition(event, state, sm): LoopState` in `src/transition.ts` is the SOLE owner of state-machine transition + `LoopState.currentState` mirroring + `phaseResults` clearing on LOOP. Extracted from the 20-line switch in `runLoop` (loop-runner.ts:224-243). `runLoop` no longer assigns `state.currentState` directly (was 7 drift sites). `applyTransition` is PURE (no I/O) — `onPhaseFailed` stays a caller-supplied callback, fixed separately at the daemon call site (daemon.ts:144 no-op) under RC-4/RC-5. `resolveTransition` stays separate (decides the event); `applyTransition` only applies an already-resolved event. Invalid events for the current state surface as errors (owned by `sm.transition`).
...
- **Recovery**: A post-execution decision point — "now that this ran and did not pass, what next?" Has variants: `healAndRetry` (run a fix command, re-run the verify phase), `failTerminal` (mark failed, broadcast). Only meaningful when a real result exists. The old inline heal block in execute-phases.ts was deleted as dead code (plan mapping never piped `healCommand`/`maxRetries` through); `healAndRetry` is defined as a recovery variant but currently has no live caller.
